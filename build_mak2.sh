...
verifyAppName()
{
	local app=$1;
	case "$app" in
		adda247 | goldweb | livedoubts | ebooks | paymentservice | erp | liveclasses | userauth | bigservice | analytics | appInstall | contentadmin | franchise | mailingservice | pushservice | ranking | testseries | timeline | Video-Streaming-server | store-elastic-search | coupon-admin | couponservice | extraservice | socialclient | newcouponadmin | ytsearch | newcouponservice | mars | Video-Streaming-server | doubts ) 
		;;
...
findDependency()
{
	...
	storefront-user)
		findAppPath commons;
        	buildPackage commons $gitpath master;
		findAppPath commons-parent;
        	buildPackage commons-parent $gitpath temp-master2;
...
if [ "$appname" = "newcouponadmin" ]; then
	...
	appnametwist="ebook";
elif [ "$appname" = "liveclasses" ]; then
        appnametwist="liveclassesstaging";
else
