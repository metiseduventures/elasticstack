while IFS= read cfdist; do  
	...
	aws cloudwatch put-metric-alarm --alarm-name ${cfdist}-Cloudfront-5xx-ErrorRate --alarm-description "${cfname} 5xx Error Rate on Cloudfront Distribution" --metric-name 5xxErrorRate --namespace AWS/CloudFront --statistic Average --period 300 --threshold 5 --comparison-operator GreaterThanOrEqualToThreshold  --dimensions --evaluation-periods 3 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-RED-ALERT  --treat-missing-data notBreaching --dimensions Name=Region,Value=Global Name=DistributionId,Value=${cfdist}
