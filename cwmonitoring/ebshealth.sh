while IFS= read env; do  
	...
	aws cloudwatch put-metric-alarm --alarm-name ${env}-EnvironmentHealthDegraded --alarm-description "${env} Environment Degraded" --metric-name EnvironmentHealth --namespace AWS/ElasticBeanstalk --statistic Maximum --period 60 --threshold 20 --comparison-operator GreaterThanOrEqualToThreshold  --dimensions "Name=EnvironmentName,Value=${env}" --evaluation-periods 8 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-RED-ALERT 
