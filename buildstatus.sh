...
data=$(mysql -u$dbuser -p$dbpass -h$dbhost $dbname -se "select deploymentid,environment,versionlabel from $dbtable where status='started'");
if [ -z "$data" ]; then
	echo "All Deployments Completed"; exit 0;
fi
