...
asg="$(aws elasticbeanstalk describe-environment-resources --environment-name $env --output=text --profile=mumbai)";
ASG=`echo "${asg}" | awk -F"\t" '$1=="AUTOSCALINGGROUPS" {print $2}'`;
groupmsg="Environment Scaling in progress";
...
while true; do
    ...
    case $opt in
		...
		aws autoscaling update-auto-scaling-group --auto-scaling-group-name $ASG --min-size 0 --max-size 0 --profile=mumbai;
		if [ $? -eq 0 ]
		then
		...
		aws autoscaling update-auto-scaling-group --auto-scaling-group-name $ASG --min-size 1 --max-size 1 --profile=mumbai;
		if [ $? -eq 0 ]
		then
