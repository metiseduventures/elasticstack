while IFS= read env; do  
	...
	aws cloudwatch put-metric-alarm --alarm-name ${env}-HighCPU --alarm-description "${env} CPU utilization exceeding 75 percent" --metric-name CPUUtilization --namespace AWS/EC2 --statistic Average --period 60 --threshold 75 --comparison-operator GreaterThanOrEqualToThreshold  --dimensions "Name=AutoScalingGroupName,Value=${ASG}" --evaluation-periods 3 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-ORANGE-ALERT
	aws cloudwatch put-metric-alarm --alarm-name ${env}-HighCPU --alarm-description "${env} CPU utilization exceeding 90 percent" --metric-name CPUUtilization --namespace AWS/EC2 --statistic Average --period 60 --threshold 90 --comparison-operator GreaterThanOrEqualToThreshold  --dimensions "Name=AutoScalingGroupName,Value=${ASG}" --evaluation-periods 3 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-RED-ALERT
	aws cloudwatch put-metric-alarm --alarm-name ${env}-EnvironmentHealthDegraded --alarm-description "${env} Environment Degraded" --metric-name EnvironmentHealth --namespace AWS/ElasticBeanstalk --statistic Maximum --period 60 --threshold 20 --comparison-operator GreaterThanOrEqualToThreshold  --dimensions "Name=EnvironmentName,Value=${env}" --evaluation-periods 5 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-RED-ALERT 
	aws cloudwatch put-metric-alarm --alarm-name ${env}-EnvironmentHealthWarning --alarm-description "${env} Environment Warning" --metric-name EnvironmentHealth --namespace AWS/ElasticBeanstalk --statistic Maximum --period 300 --threshold 15 --comparison-operator GreaterThanOrEqualToThreshold --dimensions "Name=EnvironmentName,Value=${env}" --evaluation-periods 6 --alarm-actions arn:aws:sns:us-east-1:490891715875:ADDA247-ORANGE-ALERT
	if [ ${#ALB} = 0 ];then 
