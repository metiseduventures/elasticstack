while IFS= read env; do  
	...
	draintime=`aws elasticbeanstalk describe-configuration-settings --environment-name ${env} --application-name ${app} --output=text | awk '/DeregistrationDelay/ || /ConnectionDrainingTimeout/'`
	echo $env" "$draintime;
done < "./all.txt"
